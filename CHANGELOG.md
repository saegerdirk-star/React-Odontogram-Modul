...
- Drop local FHIR codegen (`src/fhir/generated/`, `tools/generate-dental-core-types.mjs`, `fhir:generate`) and the `@cognovis/fhir-release` / `@cognovis/codegen` pins. Mapping classes and canonicals come from `@cognovis/fhir-sdk@0.11.0` (`dental-core` and `canonicals` exports). Until that package is on npm.cognovis.de, the lockfile pin is the packed local `vendor/cognovis-fhir-sdk-0.11.0.tgz`; re-pin to the registry after publish.
