...
describe("FHIR button routing", () => {
  ...
  it("uses the supplied Dental Core session codec", async () => {
    const session = createOdontogramSession({ version: "2.25", globals: {}, teeth: {} }, {
  ...
  it("uses the private component session configured through the FHIR prop", async () => {
    render(<App document={{ version: "2.25", globals: {}, teeth: {} }} fhir={{ dialect: "dental-core", exportOptions: { subject: "Patient/mira", effectiveDateTime: "2026-08-14" } }} />);
  ...
  it("shows the effective-date alert for a Dental Core session without export context", async () => {
    ...
    render(<App document={{ version: "2.25", globals: {}, teeth: { "16": { endoResection: true } } }} fhir={{ dialect: "dental-core" }} />);
