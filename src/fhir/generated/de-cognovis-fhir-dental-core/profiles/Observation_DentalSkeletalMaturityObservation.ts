...
export type DentalSkeletalMaturityObservationProfileRaw = {
...
// CanonicalURL: https://fhir.cognovis.de/dental-core/StructureDefinition/dental-skeletal-maturity-observation (pkg: de.cognovis.fhir.dental.core#0.5.0)
export class DentalSkeletalMaturityObservationProfile {
    static readonly canonicalUrl = "https://fhir.cognovis.de/dental-core/StructureDefinition/dental-skeletal-maturity-observation";
    ...
    static from (resource: Observation) : DentalSkeletalMaturityObservationProfile {
        if (!resource.meta?.profile?.includes(DentalSkeletalMaturityObservationProfile.canonicalUrl)) {
            throw new Error(`DentalSkeletalMaturityObservationProfile: meta.profile must include ${DentalSkeletalMaturityObservationProfile.canonicalUrl}`)
        }
        const profile = new DentalSkeletalMaturityObservationProfile(resource);
    ...
    static is (resource: unknown) : resource is Observation {
        ...
        return (r.meta?.profile ?? []).includes(DentalSkeletalMaturityObservationProfile.canonicalUrl);
    }

    static apply (resource: Observation) : DentalSkeletalMaturityObservationProfile {
        ensureProfile(resource, DentalSkeletalMaturityObservationProfile.canonicalUrl);
        return new DentalSkeletalMaturityObservationProfile(resource);
    }

    static createResource (args: DentalSkeletalMaturityObservationProfileRaw) : Observation {
        const resource: Observation = {
            ...
            meta: { profile: [DentalSkeletalMaturityObservationProfile.canonicalUrl] },
        }
        return resource;
    }

    static create (args: DentalSkeletalMaturityObservationProfileRaw) : DentalSkeletalMaturityObservationProfile {
        const resource = DentalSkeletalMaturityObservationProfile.createResource(args);
        return DentalSkeletalMaturityObservationProfile.apply(resource);
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "DentalSkeletalMaturityObservation"
