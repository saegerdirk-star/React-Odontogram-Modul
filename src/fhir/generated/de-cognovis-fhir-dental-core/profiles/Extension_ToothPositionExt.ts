...
// CanonicalURL: https://fhir.cognovis.de/dental-core/StructureDefinition/tooth-position (pkg: de.cognovis.fhir.dental.core#0.5.0)
export class ToothPositionExtProfile {
    static readonly canonicalUrl = "https://fhir.cognovis.de/dental-core/StructureDefinition/tooth-position";
    ...
    static from (resource: Extension) : ToothPositionExtProfile {
        const profile = new ToothPositionExtProfile(resource);
    ...
    static is (resource: unknown) : resource is Extension {
        ...
        return (resource as { url?: string }).url === ToothPositionExtProfile.canonicalUrl;
    }

    static apply (resource: Extension) : ToothPositionExtProfile {
        resource.url = ToothPositionExtProfile.canonicalUrl;
        applyFixedValue(resource, "url", "https://fhir.cognovis.de/dental-core/StructureDefinition/tooth-position");
        return new ToothPositionExtProfile(resource);
    }
    ...
    static create () : ToothPositionExtProfile {
        const resource = ToothPositionExtProfile.createResource();
        return ToothPositionExtProfile.apply(resource);
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "ToothPositionExt"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", ToothPositionExtProfile.canonicalUrl),
