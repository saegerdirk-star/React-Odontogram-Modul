...
// CanonicalURL: https://fhir.cognovis.de/dental-core/StructureDefinition/tooth-surface (pkg: de.cognovis.fhir.dental.core#0.3.0)
export class ToothSurfaceExtProfile {
    ...
    static from (resource: Extension) : ToothSurfaceExtProfile {
        const profile = new ToothSurfaceExtProfile(resource);
    ...
    static is (resource: unknown) : resource is Extension {
        ...
        return (resource as { url?: string }).url === ToothSurfaceExtProfile.canonicalUrl;
    }

    static apply (resource: Extension) : ToothSurfaceExtProfile {
        resource.url = ToothSurfaceExtProfile.canonicalUrl;
        applyFixedValue(resource, "url", "https://fhir.cognovis.de/dental-core/StructureDefinition/tooth-surface");
        return new ToothSurfaceExtProfile(resource);
    }

    static createResource () : Extension {
        const resource: Extension = {
            url: "https://fhir.cognovis.de/dental-core/StructureDefinition/tooth-surface",
        }
        return resource;
    }

    static create () : ToothSurfaceExtProfile {
        const resource = ToothSurfaceExtProfile.createResource();
        return ToothSurfaceExtProfile.apply(resource);
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "ToothSurfaceExt"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", ToothSurfaceExtProfile.canonicalUrl),
