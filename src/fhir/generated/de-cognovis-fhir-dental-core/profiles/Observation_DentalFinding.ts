...
export type DentalFindingProfileRaw = {
    code: CodeableConcept;
    subject: Reference<"Device" | "Group" | "Location" | "Patient">;
    status: ("registered" | "preliminary" | "final" | "amended" | "corrected" | "cancelled" | "entered-in-error" | "unknown");
}

// CanonicalURL: https://fhir.cognovis.de/dental-core/StructureDefinition/dental-finding (pkg: de.cognovis.fhir.dental.core#0.6.0)
export class DentalFindingProfile {
    static readonly canonicalUrl = "https://fhir.cognovis.de/dental-core/StructureDefinition/dental-finding";
    ...
    static from (resource: Observation) : DentalFindingProfile {
        if (!resource.meta?.profile?.includes(DentalFindingProfile.canonicalUrl)) {
            throw new Error(`DentalFindingProfile: meta.profile must include ${DentalFindingProfile.canonicalUrl}`)
        }
        const profile = new DentalFindingProfile(resource);
    ...
    static is (resource: unknown) : resource is Observation {
        ...
        return (r.meta?.profile ?? []).includes(DentalFindingProfile.canonicalUrl);
    }

    static apply (resource: Observation) : DentalFindingProfile {
        ensureProfile(resource, DentalFindingProfile.canonicalUrl);
        return new DentalFindingProfile(resource);
    }

    static createResource (args: DentalFindingProfileRaw) : Observation {
        const resource: Observation = {
            resourceType: "Observation",
            code: args.code,
            subject: args.subject,
            status: args.status,
            meta: { profile: [DentalFindingProfile.canonicalUrl] },
        }
        return resource;
    }

    static create (args: DentalFindingProfileRaw) : DentalFindingProfile {
        const resource = DentalFindingProfile.createResource(args);
        return DentalFindingProfile.apply(resource);
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "DentalFinding"
