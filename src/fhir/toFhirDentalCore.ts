...
const profile = (id: string) => [`${PROFILE}/${id}`];
...
function procedure(record: ToothRecord, fdi: string, payload: OdontogramExportPayload, options: FhirExportOptions): Procedure[] {
  ...
  for (const [flag, text] of [[record.endoResection, "Apicoectomy or root-end resection"], [record.fissureSealing, "Fissure sealing"]] as const) {
    if (!flag) continue;
    result.push({ resourceType: "Procedure", meta: { profile: profile("dental-procedure") }, status: "completed", code: { text }, subject: { reference: subjectReference(options) }, performedDateTime: effective(payload, options), bodySite: [{ coding: [coding(FDI_SYSTEM, fdi)] }] });
  }
  return result;
}
...
function devices(record: ToothRecord, fdi: string, options: FhirExportOptions): Device[] {
  ...
  return types.map((text, index) => ({
    ...
    extension: [{ url: `${PROFILE}/tooth-position`, valueCoding: coding(FDI_SYSTEM, fdi) }],
  }));
}
...
function observedFindings(record: ToothRecord, fdi: string, payload: OdontogramExportPayload, options: FhirExportOptions): Observation[] {
  ...
  return CHART_MAPPINGS.flatMap((mapping) => {
    ...
    return finding.valueBoolean !== undefined || finding.valueCodeableConcept ? [finding] : [];
  });
}
