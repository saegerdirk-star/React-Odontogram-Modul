...
### 📋 Prehľad
Tento projekt je interaktívny, prehliadačovo orientovaný editor odontogramu, ktorý umožňuje rýchle zaznamenávanie zubného statusu s prehľadným rozhraním. Vykresľuje vrstvené SVG šablóny zubov na reprezentáciu reštaurácií, kazu, endodontického stavu, mobility a ďalších klinických detailov, pričom poskytuje viacnásobný výber, filtre výberu a preddefinované stavové predvoľby. Každá pozícia zuba má vlastnú kresbu — šestnásť bočných pohľadov stálych zubov, dvadsať okluzálnych pohľadov a mliečny chrup — a pohľad zhora na predné zuby vôbec umožňuje zaznamenať palatinálny nález na rezáku, ktorý bočný pohľad nedokáže zobraziť.
...
- 🩻 **Kefalometria** (`odontogram-c51.2`): jeden spoločný súbor meracích bodov, nad ním merané veličiny a nad nimi metódy ako profily — nová škola je nový profil, body sa nemenia. Každá veličina nesie svoj zdroj a svoje FHIR kódovanie; norma bez publikácie sa nedodáva. Odvodené: poloha čeľustí voči lebke (typ tváre podľa Björka, harmónia, sagitálna trieda voči populačnej **aj** individuálnej norme) a rastový vzorec ako hlasovanie medzi všetkými indikátormi s doloženou normou. Hodnoty možno prevziať z tlačeného vyhodnotenia iného programu vložením textu — nič sa neuplatní bez potvrdenia Dodávajú sa štyri analýzy: **Segner/Hasund**, **Ricketts**, **Jarabak** a **Steiner**. Steiner a ďalšie pochádzajú z klinického katalógu kefalometrických analýz a neuvádzajú zdroj — norma je verejný fakt a klinik ju overí s pôvodnou literatúrou (pole `source` je interné a nikdy sa nezobrazuje). Faciálna os ukazuje, načo je toto vrstvenie — Ricketts ju uvádza ako 90 ± 3,5 a Paddenberg ako 90 ± 3,0, takže 93,3° sa číta vnútri jedného rozptylu a mimo druhého; prepis patrí profilu a meraná veličina si ponecháva vlastnú normu. Výber ich zoradí abecedne podľa preloženého názvu, s **obľúbenými** v skupine navrchu; prvá obľúbená otvára kartu, kým si niekto výslovne nevyberie. Preferencia ordinácie ako paleta výplní — stav relácie, nikdy súčasť payloadu a zámerne mimo resetu.
...
- 🔗 Retenčné prvky, ktoré držia snímateľnú náhradu na prirodzenom zube (`retention`, `retentionSide`) — tri ukotvenia, nie jedna os: **spona** vyžaduje len prítomný zub, **zásuvný spoj** a **pilier trámca** korunku. JEDNA hodnota na zub, nikdy množina. Spona sa KRESLÍ ako štvrťoblúkové rameno na korunke (bruchom k ďasnu, zrkadlené podľa oblúka); zásuvný spoj a trámec nesú charlyho značky `( G )` a `ste`. **Rozpätie trámca sa odvodzuje**, nikdy neukladá, a jeden trámec môže spočívať súčasne na implantátových aj prirodzených pilieroch
- 🎨 **Farby výplní a náhrad sú voliteľné** (Nastavenia → Farby). Každá výplňová farba je CSS premenná s dodanou farbou ako záložnou; e.max a kov-keramika sa kreslia z deväťstupňového prechodu a voľba zachová jeho svetlostný priebeh. Nastavenie ambulancie, nie súčasť dokumentu.
- 🔩 **Prázdny údaj o implantáte je medzerou len tam, kde implantát zaviedla ambulancia** (`isImplantProductGap`) — ten, s ktorým pacient prišiel, je úplný záznam. Odvodené zo vstupného vyšetrenia, nikdy neukladané.
...
- **Riadok náhrady:** kombinovaný rozbaľovací zoznam náhrady zoskupené podľa druhu (Korunka / Mostík / Inlay / Onlay / Fazeta / „Kivehető:"), pričom každý riadok sám pomenúva svoj druh a materiál (pevné možnosti `restorationType`×`restorationMaterial` plus možnosti upevnenia/snímateľné z osi `prosthesis`, obmedzené podľa druhu zuba); zaškrtávacie políčko okrajovej netesnosti korunky (len korunka/mostík); zaškrtávacie políčka miesta zlomenej korunky; prepínače potrebná korunka / potrebná výmena korunky
...
**Retenčný prvok** (`retention` + `retentionSide`; na zub, viazané podľa prvku; bez grafickej vrstvy, kreslené v prekryve mriežky):
`none`, `clasp`, `attachment`, `bar-abutment` — `retentionSide`: `none`, `mesial`, `distal`, `both`. **Teleskop** zostáva MATERIÁLOM korunky a rozpoznáva sa ako retencia
...
Konverzia FHIR je čistá voliteľná projekcia dokumentu používateľského rozhrania. Dental Core `de.cognovis.fhir.dental.core#0.6.1` je jediný FHIR kontrakt; rozpoznaná značka `odontogram-dental-core-0.6.0` zostáva čitateľná pre prázdne staršie kolekcie, neznáme značky dialektov sa odmietnu. Import, export a opätovný export zachovávajú jedenásť zdrojových osí `implantPosition`, `crownFractureType`, `orthoProgressive`, `rootResection`, `papillaLoss`, `orthoBracketSide`, `cantilever`, `endoCanals`, `rootFractureRoot`, `rootResectionRoot` a `apicalRoot`. V plánoch tieto osi používajú odkazovaný cieľový `Goal`; existujúce polia plánu naďalej používajú profilované plánované Observations. Plánovaný stav zostáva oddelený od pozorovaného a nevytvára Devices ani vykonané Procedures. Duplicitné, rozporné, chybné, nejednoznačne adresované alebo s daným zubom nezlučiteľné tvrdenia sa odmietnu. Koreňové kolíky zostávajú nezávislé cez `rootPostType`; staršie hodnoty `endo-glass-pin` a `endo-metal-pin` sa migrujú na `endo-filling` plus materiál kolíka.
...
- `retention` - čo drží snímateľnú náhradu na tomto zube (none/clasp/attachment/bar-abutment)
- `retentionSide` - strana, na ktorej retenčný prvok zaberá (none/mesial/distal/both)
...
- **PDF správa:** položka „Správa PDF…" v exportnej ponuke otvorí `ExportOptionsModal` — nastavovací dialóg (polia mena pacienta + dátumu narodenia + dátumu vyšetrenia, priamo prepojené s metadátami prípadu, pričom dátum vyšetrenia je predvolene nastavený na dnešný deň; zaškrtávacie políčka sekcií: údaje pacienta, graf odontogramu, popis odontogramu, individuálne poznámky — deaktivované, ak žiadny zub nemá poznámku — parodontálny stav, parodontálny popis) pred volaním `exportPdf(opts)`. Prázdne identifikačné pole sa vytlačí ako **„neuvedené"**, nikdy nie ako vymyslená hodnota (`odontogram-in2`): správa, ktorá *vyzerá* úplne a nesie vymyslený dátum narodenia, nie je neúplný nález, ale nesprávny — kto ju dostane do rúk, nemá ako spoznať, že dátum nepochádza od pacienta. Riadok zostáva namiesto toho, aby vypadol: chýbajúci riadok sa číta ako „tu nič nie je", označený prázdny ako „nezaznamenané". **Dátum vyšetrenia** je výnimka a naďalej sa vracia k dnešku — správa sa píše dnes, a to nie je tvrdenie o pacientovi. PDF sa zostavuje natívne cez jsPDF — vektorový text cez `.text()`, rastrové obrázky zuba/parodontálneho grafu cez `.addImage()` — **bez závislosti na svg2pdf.js**. Sekcia individuálnych poznámok sa automaticky preskočí, keď žiadny zub nemá poznámku, a obe parodontálne sekcie sa preskočia, keď je `hasAnyPerioData()` false, bez ohľadu na zaškrtávacie políčka dialógu.
...
- `src/assets/teeth-svgs/` - SVG šablóny zubov (40 súborov: jedna na pozíciu - 16 bočných pohľadov trvalých zubov, 10 mliečnych, 14 oklúznych)
