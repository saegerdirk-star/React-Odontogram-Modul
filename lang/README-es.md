...
La conversión FHIR es una proyección opcional y pura del documento de la interfaz. Dental Core `de.cognovis.fhir.dental.core#0.6.1` es el único contrato FHIR; el marcador reconocido `odontogram-dental-core-0.6.0` sigue siendo legible para colecciones antiguas vacías y se rechazan los marcadores de dialecto desconocidos. La importación, exportación y reexportación conservan los once ejes de origen `implantPosition`, `crownFractureType`, `orthoProgressive`, `rootResection`, `papillaLoss`, `orthoBracketSide`, `cantilever`, `endoCanals`, `rootFractureRoot`, `rootResectionRoot` y `apicalRoot`. En los planes, estos ejes usan un `Goal` de estado objetivo referenciado; los campos de plan existentes siguen usando Observations planificadas con perfil. El estado planificado permanece separado del observado y no crea Devices ni Procedures realizadas. Se rechazan las afirmaciones duplicadas, contradictorias, mal formadas, con dirección ambigua o incompatibles con el diente. Los postes siguen siendo independientes mediante `rootPostType`; los valores antiguos `endo-glass-pin` y `endo-metal-pin` migran a `endo-filling` más el material del poste.
