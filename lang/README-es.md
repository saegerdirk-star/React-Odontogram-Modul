...
### 📋 Descripción general
Este proyecto es un editor de odontograma interactivo basado en navegador que permite un registro rápido del estado dental con una interfaz limpia. Renderiza plantillas SVG de dientes en capas para representar restauraciones, caries, estado endodóntico, movilidad y otros detalles clínicos, proporcionando selección múltiple, filtros de selección y estados predefinidos. Cada posición dentaria tiene su propio dibujo — dieciséis vistas laterales permanentes, veinte vistas oclusales y la dentición temporal — y la vista superior de los dientes anteriores es lo que permite registrar un hallazgo palatino en un incisivo, algo que la vista lateral no puede mostrar.
...
- 🩻 **Cefalometría** (`odontogram-c51.2`): un único repertorio de puntos, las medidas definidas sobre él y los análisis como perfiles por encima — una escuela nueva es un perfil nuevo, los puntos no se mueven. Cada medida lleva su fuente y su codificación FHIR; una norma sin publicación no se entrega, la medida se registra sin objetivo. Se derivan la posición de los maxilares respecto al cráneo (tipo facial según Björk, armonía, clase sagital frente a la norma poblacional **y** a la individual) y el patrón de crecimiento como votación entre todos los indicadores con norma documentada. Los valores pueden tomarse de la evaluación impresa de otro programa pegando su texto — nada se aplica sin confirmación Se incluyen cuatro análisis: **Segner/Hasund**, **Ricketts**, **Jarabak** y **Steiner**. Steiner y los demás proceden de un catálogo clínico de análisis cefalométricos y no citan fuente — la norma es un hecho público y el clínico la verifica con la literatura original (el campo `source` es interno y nunca se muestra). El eje facial muestra para qué sirve esta estratificación: Ricketts lo da como 90 ± 3,5 y Paddenberg como 90 ± 3,0, de modo que 93,3° queda dentro de una dispersión y fuera de la otra; la sobrescritura pertenece al perfil y la medida conserva su propia norma. El selector los ordena alfabéticamente por su nombre traducido, con los **favoritos** en un grupo arriba; el primer favorito abre la tarjeta mientras nadie haya elegido explícitamente. Una preferencia de la consulta, como la paleta de restauraciones: estado de sesión, nunca parte del payload y deliberadamente fuera del reinicio.
- 🖐️ **Edad ósea** (`odontogram-c51.4`): cuánto crecimiento queda, leído de dos formas y mantenido separado — maduración vertebral cervical (CVM, 6 estadios) en la misma telerradiografía y SMI de Fishman (11 estadios) en la radiografía de mano. Los once SMI se asignan a los seis estadios CVM en pares fijos, así que ambos dan el mismo rango de crecimiento restante; un CVM leído directamente prevalece sobre el derivado de la mano, y una discrepancia se indica, no se resuelve. Junto al patrón de crecimiento cefalométrico.
- 📸 **Análisis fotostático — Powell** (`odontogram-c51.3`): ángulos de foto de perfil, integrados en la tarjeta cefalométrica pero marcados como MEDIO distinto: cada medida y el perfil llevan `medium: "photo"`, y el selector agrupa por él (telerradiografía vs. fotostática), de modo que el registro indica si un valor de tejidos blandos se leyó en la radiografía o en la foto.
...
- **Fila de restauración:** el menú desplegable combinado de restauración agrupado por tipo (Corona / Puente / Incrustación / Onlay / Carilla / «Kivehető:»), con cada línea nombrando su propio tipo y material (opciones fijas `restorationType`×`restorationMaterial` más las opciones de anclaje/removible de `prosthesis`, condicionadas por el tipo de diente); casilla de filtración marginal de corona (solo corona/puente); casillas de ubicación de corona fracturada; interruptores de corona necesaria / reemplazo de corona necesario
...
**Material del poste radicular (dientes permanentes, independiente de `endo`):**
...
La conversión FHIR es una proyección opcional y pura del documento de la interfaz. Dental Core `de.cognovis.fhir.dental.core#0.6.0`, procedente de la proyección exacta `@cognovis/fhir-release@0.2.4`, es el único contrato FHIR. La versión 3 elimina la representación anterior ajena a Dental Core y toda selección de dialecto en tiempo de ejecución; los Bundles externos, no admitidos o mal formados se rechazan explícitamente. Los postes radiculares permanecen independientes mediante `rootPostType`. El JSON antiguo y los valores Dental Core anteriores `endo-glass-pin` / `endo-metal-pin` migran a `endo-filling` más el material del poste; la salida nueva nunca vuelve a combinar ambos ejes.

**Modo en vivo con Aidbox (desarrollo, desde 2.50.0):**

Un segundo punto de entrada del servidor de desarrollo, `live.html` (`src/live`), carga la ficha de un paciente directamente desde un Aidbox en ejecución, la renderiza en la interfaz habitual a través de la API de sesión descrita arriba, y escribe los cambios de vuelta como recursos Dental Core bajo ids deterministas, de modo que un nuevo guardado actualiza en lugar de duplicar. Se configura mediante un `.env` excluido del control de versiones (copia de `.env.example`) que nombra **únicamente un cliente de máquina con alcance restringido** — nunca una credencial de administrador. Es una herramienta de desarrollo, no forma parte del paquete publicado: los paquetes del SDK `@polaris` son devDependencies, `dependencies` no cambia, y ni `src/live` ni `live.html` se publican. La configuración, la mecánica de carga/guardado y la diferencia documentada respecto al dialecto del adaptador charly están en [`docs/aidbox-live-mode.md`](../docs/aidbox-live-mode.md). Ten en cuenta que instalar las devDependencies de este repositorio ahora requiere una credencial para `npm.cognovis.de` (véase el documento); `npm ci --omit=dev` y el consumo del paquete publicado no la requieren.
...
- `rootPostType` - material del poste radicular (`none`, `glass-fiber`, `metal`), independiente de `endo`
...
- **Informe PDF:** el elemento "Informe PDF…" del menú de exportación abre `ExportOptionsModal` — un diálogo de ajustes (campos de nombre del paciente + fecha de nacimiento + fecha de examen, conectados directamente a los metadatos del caso, con la fecha de examen predeterminada al día de hoy; casillas de sección: datos del paciente, odontograma, descripción del odontograma, notas individuales — desactivada cuando ningún diente tiene una nota —, estado periodontal, descripción periodontal) antes de llamar a `exportPdf(opts)`. Un campo de identidad vacío se imprime como **«no especificado»**, nunca como un valor inventado (`odontogram-in2`): un informe que *parece* completo y lleva una fecha de nacimiento inventada no es un hallazgo incompleto sino uno erróneo — quien lo recibe no puede saber que la fecha no proviene del paciente. La línea permanece en lugar de omitirse: una línea ausente se lee como «aquí no hay nada», una línea etiquetada y vacía como «no registrado». La **fecha del examen** es la excepción y sigue recurriendo a hoy: un informe se redacta hoy, y eso no es una afirmación sobre el paciente. El PDF se ensambla de forma nativa con jsPDF — texto vectorial mediante `.text()`, imágenes rasterizadas del odontograma/gráfico periodontal mediante `.addImage()` — **sin dependencia de svg2pdf.js**. La sección de notas individuales se omite automáticamente cuando ningún diente tiene una nota, y las dos secciones periodontales cuando `hasAnyPerioData()` es `false`, independientemente de las casillas del diálogo.
