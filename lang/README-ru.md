...
- 🩻 **Цефалометрия** (`odontogram-c51.2`): единый набор точек, над ним измеряемые величины, а над ними методы как профили — новая школа это новый профиль, точки не меняются. Каждая величина несёт свой источник и своё FHIR-кодирование; норма без публикации не поставляется. Выводятся: положение челюстей относительно черепа (тип лица по Бьёрку, гармония, сагиттальный класс относительно популяционной **и** индивидуальной нормы) и характер роста как голосование всех показателей с документированной нормой. Значения можно перенести из печатной оценки другой программы вставкой текста — ничто не применяется без подтверждения Поставляются два анализа: **Хасунд** (относительно передней черепной ямки) и **Рикеттс** (относительно франкфуртской горизонтали, нормы для взрослых). Лицевая ось показывает, зачем нужно это разделение: Рикеттс даёт её как 90 ± 3,5, а Падденберг как 90 ± 3,0, поэтому 93,3° попадает внутрь одного разброса и за пределы другого; переопределение принадлежит профилю, а измерение сохраняет собственную норму.
