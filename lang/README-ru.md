...
Полная поверхность API (≈ 44 функции + типы, такие как `OdontogramSummary`, `OdontogramThemeConfig`, `OdontogramPlugin`, `PerioViewMode`, …) полностью типизирована во встроенных объявлениях.
...
- **Только ESM** — пакет публикует основной ES-модуль (`dist/odontogram.js`) с соответствующими декларациями типов (`dist/index.d.ts`). Он ориентирован на разрешение модулей сборщиком; сборки CommonJS нет.
...
- 🔗 Aidbox Dental Core через `@cognovis/fhir-sdk`: хост передаёт gateway; сессия загружает и сохраняет. JSON-экспорт/импорт статуса с миграциями
...
Преобразование FHIR — это чистая необязательная проекция документа интерфейса. Dental Core `de.cognovis.fhir.dental.core#0.7.1` является единственным контрактом FHIR; распознаваемый маркер `odontogram-dental-core-0.6.0` по-прежнему принимается для пустых устаревших коллекций, а неизвестные маркеры диалектов отклоняются. Импорт, экспорт и повторный экспорт сохраняют одиннадцать исходных осей: `implantPosition`, `crownFractureType`, `orthoProgressive`, `rootResection`, `papillaLoss`, `orthoBracketSide`, `cantilever`, `endoCanals`, `rootFractureRoot`, `rootResectionRoot` и `apicalRoot`. В планах эти оси используют ссылочный `Goal` целевого состояния; существующие поля плана по-прежнему используют профилированные плановые Observations. Плановое состояние отделено от наблюдаемого и не создаёт Devices или выполненные Procedures. Дублирующиеся, противоречивые, некорректные, неоднозначно адресованные или несовместимые с зубом утверждения отклоняются. Корневые штифты остаются независимыми благодаря `rootPostType`; прежние значения `endo-glass-pin` и `endo-metal-pin` мигрируют в `endo-filling` плюс материал штифта.

**Живой режим Aidbox (разработка, начиная с 2.50.0):**

Вторая точка входа dev-сервера, `live.html` (`src/live`), загружает карту одного пациента напрямую из изолированного Reetfurt local-UAT Aidbox, отображает её в обычной оболочке через описанный выше API сессии и записывает изменения обратно как ресурсы Dental Core с детерминированными id, так что повторное сохранение обновляет, а не дублирует. Сначала запустите именованный экземпляр Reetfurt (`POLARIS_DIR=$HOME/code/polaris/platform bun run uat:local up --instance <id>` в checkout mvz-reetfurt), затем выполните `npm run live:provision -- --instance <id>`: это создаст ограниченного машинного клиента `odontogram-live` и запишет исключённый из контроля версий `.env`. Учётные данные администратора никогда не помещают в `VITE_*`. Это инструмент разработки, не входящий в опубликованный пакет: `@cognovis/fhir-sdk` — runtime-зависимость для классов Dental Core; `src/live` не публикуется, и ни `src/live`, ни `live.html` не публикуются. Настройка, механика загрузки/сохранения и задокументированное отличие от диалекта адаптера charly описаны в [`docs/aidbox-live-mode.md`](../docs/aidbox-live-mode.md). Обратите внимание: установка devDependencies этого репозитория теперь требует учётных данных для `npm.cognovis.de` (см. документ); `npm ci --omit=dev` и использование опубликованного пакета этого не требуют.
...
- `src/fhir/` — единственная граница Dental Core для HL7 FHIR R4: кодек `toFhirDentalCore.ts`/`fromFhirDentalCore.ts` над `@cognovis/fhir-sdk`, `dentalCoreContract.ts` и вспомогательный `dentalCoreLocalCoding.ts`
