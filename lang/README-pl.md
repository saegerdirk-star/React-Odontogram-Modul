...
- 👑 Odbudowy według typu × materiału: korona / wkład (inlay) / nakład (onlay) / licówka / most z e.max, złota, gradii, cyrkonu, metalu, metalowo-ceramicznego, teleskopowego lub tymczasowego (nakład dostępny tylko w widoku okluzyjnym) — wybierane z jednego połączonego, niskoklikowego selektora „Korona – …”; istniejące korony `metal` migrują automatycznie do `metal-ceramic` (metalowo-ceramicznej); implanty korzystają z tego samego modelu typ × materiał, złożonego z warstwą łącznika implantu. Selektor jest zawężany wg rodzaju zęba: implant oferuje tylko koronę/most (plus pięć opcji łącznikowych opisanych poniżej); ząb brakujący/luka oferuje tylko przęsło mostowe (plus protezę ruchomą częściową/całkowitą); podłoże `radix` całkowicie ukrywa kontrolkę odbudowy (na pozostałości korzenia nie można zapisać żadnej odbudowy)
...
- 🩻 **Cefalometria** (`odontogram-c51.2`): jeden wspólny zasób punktów pomiarowych, nad nim wielkości mierzone, a nad nimi metody jako profile — nowa szkoła to nowy profil, punkty pozostają. Każda wielkość niesie swoje źródło i kodowanie FHIR; norma bez publikacji nie jest dostarczana. Wyprowadzane: położenie szczęk względem czaszki (typ twarzy wg Björka, harmonia, klasa strzałkowa wobec normy populacyjnej **i** indywidualnej) oraz wzorzec wzrostu jako głosowanie wszystkich wskaźników z udokumentowaną normą. Wartości można przejąć z wydrukowanej oceny innego programu przez wklejenie tekstu — nic nie zostaje zastosowane bez potwierdzenia Dostarczane są cztery analizy: **Segner/Hasund**, **Ricketts**, **Jarabak** i **Steiner**. Steiner i pozostałe pochodzą z klinicznego katalogu analiz cefalometrycznych i nie podają źródła — norma jest faktem publicznym, a klinicysta weryfikuje ją z oryginalną literaturą (pole `source` jest wewnętrzne i nigdy nie jest pokazywane). Oś twarzowa pokazuje, po co jest to warstwowanie — Ricketts podaje ją jako 90 ± 3,5, a Paddenberg jako 90 ± 3,0, więc 93,3° mieści się w jednym rozrzucie, a wykracza poza drugi; nadpisanie należy do profilu, a miara zachowuje własną normę. Lista sortuje je alfabetycznie według przetłumaczonej nazwy, z **ulubionymi** w grupie na górze; pierwsza ulubiona otwiera kartę, dopóki nikt nie wybrał wprost. Preferencja gabinetu jak paleta wypełnień — stan sesji, nigdy część payloadu i celowo poza resetem.
...
- **Wiersz odbudowy:** połączona lista rozwijana odbudowy pogrupowane wg rodzaju (Korona / Most / Inlay / Onlay / Licówka / „Kivehető:”), gdzie każdy wiersz sam nazywa swój rodzaj i materiał (stałe opcje `restorationType`×`restorationMaterial` plus opcje łącznikowe/ruchome `prosthesis`, zawężane wg rodzaju zęba); pole wyboru nieszczelności brzeżnej korony (tylko korona/most); pola wyboru lokalizacji złamanej korony; przełączniki korona wymagana / wymiana korony konieczna
