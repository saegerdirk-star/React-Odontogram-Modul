...
- 🧭 **Die Kieferorthopädie ist die dritte klinische Ansicht** (`odontogram-c51`): ein Umschalter `Odontogram | Parodontalstatus | KFO` (`#appViewToggle`) trägt die beiden folgenden Karten. Das Odontogramm wird dabei nie ausgehängt, nur ausgeblendet — ein Ansichtswechsel kann den Befund also nicht stören. Die KFO-Ansicht hat kein eigenes Dialogfenster und bleibt deshalb auch dort ein Feld des Umschalters, wo der Parodontalstatus als Popup eingestellt ist; dann fällt nur dessen Feld weg.
